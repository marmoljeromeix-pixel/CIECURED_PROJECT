...
function submitReport(){
  ...
  var reportBody = 'code=' + encodeURIComponent(code) + '&category=' + encodeURIComponent(cat) + '&message=' + encodeURIComponent(desc || '(No description provided)');
...
function openCase(code){
 ...
 document.getElementById('threadCode').textContent = c.code;
  ...
  // Also check the shared admin backend for any staff replies.
  syncCaseFromServer(code);
}
...
function sendReply(e){
  ...
  // see it on their dashboard too.
  var replyBody = 'code=' + encodeURIComponent(CURRENT_CASE) + '&text=' + encodeURIComponent(text);
...
// Checks the shared admin backend for a case, and adds any staff
// replies that aren't in the local Inbox yet. This is what makes a
// staff reply from the admin dashboard show up here.
function syncCaseFromServer(code){
  fetch('get_report.php?code=' + encodeURIComponent(code))
    ...
    .then(function(data){
      ...
      if(!c.serverSyncedCount){
        c.serverSyncedCount = 0;
      }

      for(var i = c.serverSyncedCount; i < data.messages.length; i++){
        var m = data.messages[i];
        if(m.from === 'staff'){
          c.messages.push({ from:'team', text:m.text, time:m.time });
          if(c.status === 'received' || c.status === 'review'){
            c.status = 'replied';
          }
        }
      }
      c.serverSyncedCount = data.messages.length;

      if(CURRENT_CASE === code){
        renderThreadMessages();
        var statusEl = document.getElementById('threadStatus');
        statusEl.textContent = STATUS_LABEL[c.status];
        statusEl.className = 'thread-status status-pill ' + STATUS_CLASS[c.status];
    ...
    .catch(function(){
      // Offline, or the report doesn't exist on the server yet — ignore quietly.
    });
}
